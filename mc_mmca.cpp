...
void run_mc(ParamList &param, ostream &out)
{
	...
	// 参数绑定
	lambda = param["lambda"], delta = param["delta"], gamma = param["gamma"], beta = param["beta"], sigma = param["sigma"], mu = param["mu"], m1 = param["m1"], m21 = param["m21"], m22 = param["m22"], m23 = param["m23"], p1 = param["p1"], p2 = param["p2"]*param["p1"], p3 = param["p3"]*param["p2"]*param["p1"];
	for (l = 0; l < R; l++) // 20
	{
		i = s = v1 = v2 = v3 = 0;
		u = a_num = 0;
	...
	out <<"method=mc "<<param.desc() << " v1=" << c_v1 << " v2=" << c_v2 << " v3=" << c_v3 << " i=" << c_i << " s=" << c_s << endl;
}

double run_mmca(TreeRoot *root, ParamList &param, ostream &out)
{
	...
	double mu = param["mu"];
	double _gamma = param["gamma"];
	double beta = param["beta"];
	double q1 = param["p1"];
	double q2 = q1 * param["p2"];
	double q3 = q2 * param["p3"];
	double m1 = param["m1"];
	double m21 = param["m21"];
	double m22 = param["m22"];
	double m23 = param["m23"];
	int i, j, t, round;
	// 初始化
	for (t = 0; t < T; t++)
	{
	...
	c_v2 = c_v2 / R / mean_time;
	c_v3 = c_v3 / R / mean_time;
	out <<"method=mmca "<< param.desc() << " v1=" << c_v1 << " v2=" << c_v2 << " v3=" << c_v3 << " i=" << c_i << " s=" << c_s << endl;
	return c_i;
}